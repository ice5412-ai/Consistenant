...
namespace DigitalRubyShared
{
    ...
    public class FingersDragDropComponentScript : MonoBehaviour
    {
        ...
        /// </summary>
        public LongPressGestureRecognizer LongPressGesture => InputManager.Instance.LongPressGesture;
        ...
        private void LongPressGestureUpdated(DigitalRubyShared.GestureRecognizer r)
        {
            ...
            var ray = camera.ScreenPointToRay(new Vector3(r.FocusX, r.FocusY, 0.0f));
            ...
            if (isDragging && r.State == GestureRecognizerState.Executing)
            {
                Vector3 gestureScreenPoint = new Vector3(r.FocusX, r.FocusY, panZ);
                Vector3 gestureWorldPoint =
                    ScreenToWorldPoint(camera, gestureScreenPoint.x, gestureScreenPoint.y,
                        gestureScreenPoint.z) + panOffset;

                gestureWorldPoint.y = transform.position.y;
                gestureWorldPoint.z = 0;
            ...
            if (isDragging && r.State == GestureRecognizerState.Ended)
            {
                isDragging = false;
                transform.localScale /= DragScale;
